...
function drawModelCompChart() {
  ...
  const pad = { top:40, right:80, bottom:70, left:60 };
...
function computeReliability(v, t, p, h, rul) {
  ...
  const delta_t = 24;
  const R = Math.exp(-lambda * delta_t);
  const F = 1 - R;
...
function runPrediction() {
  ...
  // Health Index bar
  const hiBar = document.getElementById('hiBar');
  if (hiBar) hiBar.style.width = (HI*100)+'%';
  const hiVal = document.getElementById('hiVal');
  if (hiVal) hiVal.textContent = HI.toFixed(3);

  // Details
  const degScoreEl = document.getElementById('degScore');
  if (degScoreEl) degScoreEl.textContent = typeof S !== 'undefined' ? S.toFixed(4) : deg.toFixed(4);
