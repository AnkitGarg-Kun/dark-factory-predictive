...
function computeMLRUL(type, vib, temp, press, hrs) {
  ...
  if (!MODEL) {
    ...
    const RUL_phys = 50 + 950 * Math.pow(Math.max(0, HI), 1.4);
    const { rul: RUL_th } = computeTheoreticalRUL(type, vib, temp, press, hrs);
    const rul = Math.max(51, Math.min(999, Math.round(0.8 * RUL_phys + 0.2 * RUL_th)));
    return { rul, HI, deg };
  }
